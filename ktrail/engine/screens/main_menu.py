...
from PyQt5.QtCore import Qt, QPropertyAnimation, QEasingCurve, QTimer, QPointF, QRegExp
...
class MainMenu(QWidget):
    def __init__(self, parent=None):
        ...
        # Отложенный запуск init_username_ui для корректной позиции
        QTimer.singleShot(100, self.init_username_ui)
    ...
    def init_username_ui(self):
        # Создание поля ввода и метки через create_label
        self.username_label = self.create_label(self.username, font_size=24, x=0, y=0, w=180, h=40)
        ...
        # Перемещение по координатам после отрисовки окна
        QTimer.singleShot(100, self.position_username_elements)

        # Добавление в список для анимации
        self.widgets_to_restore.append(self.username_label)

    def position_username_elements(self):
        x = self.width() - 200
        y = self.height() - 50
        self.username_label.move(x, y)
        self.username_edit.move(x, y)
    ...
    def edit_username(self, event):
        ...
        self.position_username_elements()
